...
enum CameraCurve
{ 
	AZIMUTH=0, ELEVATION, DOLLY, TWIST, LOOKAT_X, LOOKAT_Y, LOOKAT_Z, FOV, NEARCP, FARCP, NUM_CAM_CURVES
};
...
public:
    ...
    inline void setNearCP( float nearCP )
    { mNearCP = nearCP; mDirtyTransform = true; }
    inline float getNearCP() const
    { return mNearCP; }

    inline void setFarCP( float farCP )
    { mFarCP = farCP; mDirtyTransform = true; }
    inline float getFarCP() const
    { return mFarCP; }
