...
float Curve::evaluateCurveAt(const float x) const
{
	...
	if (m_ptvEvaluatedCurvePts.size() == 0)
...
{
	if (iCtrlPt < m_ptvCtrlPts.size() && m_ptvCtrlPts.size() > 1) {
...
{
	for (int i = m_ptvCtrlPts.size()-1; i >= 0  &&  m_ptvCtrlPts.size() > 1; --i) { 
...
void Curve::reevaluate() const
{
	if (m_bDirty) {
		if (m_pceEvaluator) {
			if(m_ptvCtrlPts.size() <= 1)
			{
				m_pceEvaluator->handleEdgeCases(m_ptvCtrlPts, 
...
void Curve::set_default_value(float new_def)
{
