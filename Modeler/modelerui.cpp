...
// Initializing the UI to set the resolution
int ModelerUI::userHeight = 480;
int ModelerUI::userWidth = 640;
Fl_Input* ModelerUI::heightInput = NULL;
Fl_Input* ModelerUI::widthInput = NULL;
bool ModelerUI::userResolution = false;
...
inline void ModelerUI::cb_setResolution_i(Fl_Menu_*, void*)
{
	int width, height;
	width = height = 0;
	...
	do {
		szWidth = fl_input("New Animation Width (in pixels) (1 ~ 2500)", "");

		if (szWidth) {
			width = atoi(szWidth);
			ModelerUI::userWidth = width;
		}
	} while (width && (width < 1 || width > MAX_RES));
	do {
		szHeight = fl_input("New Animation Height (in pixels) (1 ~ 2500)", "");

		if (szHeight) {
			height = atoi(szHeight);
			ModelerUI::userHeight = height;
		}
	} while (height && (height < 1 || height > MAX_RES));
	ModelerUI::userResolution = true;
	show();
}

void ModelerUI::cb_setResolution(Fl_Menu_* o, void* v)
{
	((ModelerUI*)(o->parent()->user_data()))->cb_setResolution_i(o,v);
}
...
void ModelerUI::controlValue(int iControl, float fVal)
{
	...
	m_pwndGraphWidget->curve(iControl)->set_default_value(fVal);
	if (m_pcbfValueChangedCallback)
		m_pcbfValueChangedCallback();
}
...
m_nativeChooser(NULL)
{
	...
	m_pmiSetResolution->callback((Fl_Callback*)cb_setResolution);
	...
	ModelerUI::userWidth = 640;
	ModelerUI::userHeight = 480;
...
void ModelerUI::show()
{
	...
	// if the user specifies a resolution
	if(userResolution)
	{
		m_pwndModelerWnd->resize(40 + m_pwndMainWnd->w(), 30,
			ModelerUI::userWidth, ModelerUI::userHeight);
		m_pwndModelerWnd->show();
	}
	// else use the default resolution
	else
	{
		m_pwndModelerWnd->resize(40 + m_pwndMainWnd->w(), 30,
